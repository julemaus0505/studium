...
public class BearbeitenEintrag extends JDialog {
	...
	// NEU für die Anzeige Datensatz
	private JLabel datensatzLabel;
	private int datensatzGroesse;
	private int datensatzAnfang;

	// für die Aktionen
	private MeineAktionen loeschenAct;
	private MeineAktionen vorAct;
	private MeineAktionen zurueckAct;
	private MeineAktionen startAct;
	private MeineAktionen endeAct;
	private MeineAktionen aktualisierenAct;
	...
	class MeineAktionen extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getActionCommand().equals("vor")) {
				ganzVor();

			} else if (event.getActionCommand().equals("zurueck")) {
				ganzZurueck();

			} else if (event.getActionCommand().equals("einenvor")) {
				einenVor();

			} else if (event.getActionCommand().equals("einenzurueck")) {
				einenZurueck();

			} else if (event.getActionCommand().equals("loeschen")) {
				loeschen();

			} else if (event.getActionCommand().equals("aktualisieren")) {
				aktualisieren();
			}
		}
	}
	...
	public BearbeitenEintrag(JFrame parent, boolean modal) {
		...
		loeschenAct = new MeineAktionen("Datensatz löschen", new ImageIcon("src/icons/Delete24.gif"),
				"Löscht den aktuellen Datensatz", null, "loeschen");
		vorAct = new MeineAktionen("Einen Datensatz weiter", new ImageIcon("src/icons/Forward24.gif"),
				"Blättert einen Datensatz weiter", null, "einenvor");
		zurueckAct = new MeineAktionen("Einen Datensatz zurück", new ImageIcon("src/icons/Back24.gif"),
				"Blättert einen Datensatz zurück", null, "einenzurueck");
		startAct = new MeineAktionen("Zum ersten Datensatz", new ImageIcon("src/icons/Front24.gif"),
				"Geht zum ersten Datensatz", null, "vor");
		endeAct = new MeineAktionen("Zum letzten Datensatz", new ImageIcon("src/icons/End24.gif"),
				"Geht zum letzten Datensatz", null, "zurueck");
		aktualisierenAct = new MeineAktionen("Änderungen speichern", new ImageIcon("src/icons/Save24.gif"),
		...
		// sqlAbfrage = "SELECT * FROM adressen WHERE nachname = " + suchen;
	...
	private JPanel initGui() {
		...
		// NEU für Datensatz (nur Anzeige)
		tempPanel.add(new JLabel("Datensatz"));
		datensatzLabel = new JLabel();
		tempPanel.add(datensatzLabel);
		return tempPanel;
	}
	...
	private void datenLesen() {
		...
		try {
			...
			// NEU liefert den Anfang der Dantensätze
			datensatzAnfang = ergebnisMenge.getRow();

			// NEU zeigt das Label und die Datensätze im Fenster an
			datensatzLabel.setText(datensatzAnfang + " von " + datensatzGroesse);
	...
	private void einenVor() {
		try {
			...
			// NEU Abfrage das es nicht der letzte ist
	...
	private void einenZurueck() {
		try {
			...
			// NEU Abfrage das es nicht der erste ist
	 ...
	 * Die Methode ändert den Zustand der vor und zurück bez. start und Ende Actions, 
	 * anhand der Ergebnismenge ob es der erste oder der letzte Eintrag ist.
	 * 
	 * @throws SQLException wird geworfen, wenn es bei der 
	 * Abfrage der Position zu einem Problem kommt.
	 */
	private void zustandActAendern() throws SQLException {
		zurueckAct.setEnabled(!ergebnisMenge.isFirst());
		vorAct.setEnabled(!ergebnisMenge.isLast());
		startAct.setEnabled(!ergebnisMenge.isFirst());
		endeAct.setEnabled(!ergebnisMenge.isLast());

	}
	
	/**
	 * Beschreibung der Funktionsweise:
	 * Die Methode ermittelt die Anzahl der Datensätze.
	 * 
	 * @throws SQLException wird geworfen, wenn das ermitteln 
	 *                      der Anzahl der Datensätze zu Problemen führt.
	 */
	private void ermittelGroesse() throws SQLException {
		datensatzGroesse = 0;
		ergebnisMenge.last();
		datensatzGroesse = ergebnisMenge.getRow();
		ergebnisMenge.first();
	}
}
