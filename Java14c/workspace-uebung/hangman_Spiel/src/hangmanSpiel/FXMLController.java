...
public class FXMLController {
	...
	private void neuesWort() {
		...
		// die Wortliste
		String[] woerter = { "Test", "Automobil", "Versuch", "Hund", "Katze", "Ziege", "Maus", "Elefant",
				"Isopropylalkohol", "Schwimmbad" };
	...
	private void gewinnerOderNicht() {
		...
		if (restDurchlauefe == 0) {
			...
			alertVerloren.setTitle("Hangeman Spiel");
			alertVerloren.setContentText("Das gesuchte Wort war: " + suchwort);
			alertVerloren.setHeaderText("Sie haben leider verloren.");
			alertVerloren.showAndWait();
			Platform.exit();
		}
		// ist das Wort erraten worden?
		if (anzeige.toString().equals(suchwort)) {
			Alert alertGewonnen = new Alert(AlertType.INFORMATION);
			alertGewonnen.setTitle("Hangeman Spiel");
			alertGewonnen.setContentText("Hurra! Sie haben gewonnen!");
			alertGewonnen.showAndWait();
			Platform.exit();
		}
	}
	...
	private void erhoeheFehler() {
		...
		case 4:
			...
			break;
		case 5:
			gc.strokeOval(80, 50, 40, 40);
			break;
		case 6:
			gc.strokeLine(130, 60, 130, 120);
			// gc.strokeLine(130, 60, 130, 100);
			break;
		case 7:
