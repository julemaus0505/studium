...
public class MiniText extends JFrame {
	...
	// für die Aktionen
	// Neue Klassenvariablen hinzugefügt cutAct, copyAct, pasteAct, infoAct;
	// cutAct für das Ausschneiden, copyAct für das Kopieren, pasteAct für das
	// Einfügen, und infoAct für das Info Fenster
	private Action oeffnenAct, speichernAct, beendenAct, neuAct, druckenAct, speichernUnterAct, webOeffnenAct,

			// Dies gehört zu Teilaufgabe 2.
			cutAct, copyAct, pasteAct,

			// Dies gehört zu Teilaufgabe 1.
			infoAct;

	// für die Datei
	private File datei;
	private File dateiLokal;
	...
	class MeineAktionen extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			else if (event.getActionCommand().equals("webladen")) {
				webLaden();
			}

			else if (event.getActionCommand().equals("drucken")) {
			...
			// Dies gehört zu Teilaufgabe 1.
			// Listener für die Info und Hilfe Action setzen
	...
	public MiniText(String text) {
		...
		druckenAct = new MeineAktionen("Drucken...", new ImageIcon("icons/print24.gif"), "Druckt das aktuelle Dokument",
		...
		// Dies gehört zu Teilaufgabe 1.
		// Neue Aktion infoAct hinzugefügt
		infoAct = new MeineAktionen("Info...", new ImageIcon("icons/information24.gif"), "Information", null, "info");
	...
	public void menu() {
		...
		// Dies gehört zu Teilaufgabe 1.
		// Neues Menü Hilfe erstellen
		JMenu dateiHilfe = new JMenu("Hilfe");

		// Änderung
		// Dies gehört zu Teilaufgabe 1.
		// das Öffnen für den Dialog
		dateiHilfe.add(infoAct);
		...
		// Neu das Gesamte Menü von Hilfe hinzufügen
		menue.add(dateiHilfe);
		this.setJMenuBar(menue);
	}
	...
	public JToolBar symbolleiste() {
		...
		// Dies gehört zu Teilaufgabe 1.
		// Änderung das Symbol Information
		leiste.add(infoAct);
		...
		// Dies gehört zu Teilaufgabe 2.
		// Noch weitere Aktionen hinzugefügt
		// Erstellen der Acts für Cut, Copy & Paste
		cutAct = new DefaultEditorKit.CutAction();
		cutAct.putValue(Action.SHORT_DESCRIPTION, "Ausschneiden und in die Zwischenablage legen");
		cutAct.putValue(Action.LARGE_ICON_KEY, new ImageIcon("icons/cut24.gif"));
		leiste.add(cutAct);

		// Dies gehört zu Teilaufgabe 2.
		// Copy
		copyAct = new DefaultEditorKit.CopyAction();
		copyAct.putValue(Action.SHORT_DESCRIPTION, "Kopieren und in die Zwischenablage legen");
		copyAct.putValue(Action.LARGE_ICON_KEY, new ImageIcon("icons/copy24.gif"));
		leiste.add(copyAct);

		// Dies gehört zu Teilaufgabe 2.
		// Paste
		pasteAct = new DefaultEditorKit.PasteAction();
		pasteAct.putValue(Action.SHORT_DESCRIPTION, "Inhalt aus der Zwischenablage einfügen");
		...
		// an den Konstruktor von StyledEditorKit.AlignmentAction() wird eine
	...
	private void dateiNeu() {
		...
		if (JOptionPane.showConfirmDialog(this, "Wollen Sie wirklich ein neues Dokument anlegen?", "Neues Dokument",
			...
			// Dies gehört zu Teilaufgabe 3.
			// Änderung hier wird der Title in der Titleleiste angezeigt
			setTitle("Mini Text Ohne Namen");

			// den Text löschen
			eingabeFeld.setText("");

			// datei wieder auf null setzen, damit klar ist, dass die Datei noch nicht
			// gespeichert wurde
			datei = null;
		}
	}
	...
	private void information() {
		...
		// Änderung wenn auf dem Info Symbol oder auf das Menü Hilfe und dann info
		// geklickt wurden da
		if (infoAct.equals(infoAct)) {
			JOptionPane.showMessageDialog(null, "MiniText Version 4.0. \nProgrammiert von: Julia Petersen 2020", "Info",
	...
	public void dateiLaden() {
		...
		dateiLokal = dialog.oeffnenDialogZeigen();

		// Dies gehört zu Teilaufgabe 3.
		setTitle("Mini Text " + dateiLokal.getName() + "  " + dateiLokal);
	...
	private void dateiSpeichern() {
		...
		if (datei == null) {
			...
			// Dies gehört zu Teilaufgabe 3.
			// Änderung hier wird der DateiPfad in der Titleleiste angezeigt
			setTitle(" Mini Text " + datei.getName() + "  " + datei);
	...
	// die Methode öffnet den Speichern-Dialog und ruft die Methode dateiSpeichern()
	// auf,
	...
	private void drucken(boolean dialogZeigen) {
		try {
			...
			JOptionPane.showMessageDialog(this, "Beim Drucken hat es ein Problem gegeben.", "Fehler",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void webLaden() {
		...
		if (adresse != null) {
			...
			// Dies gehört zu Teilaufgabe 3.
			// Änderung hier wird die URL in der Titleleiste angezeigt
			setTitle(" Mini Text  " + adresse);
